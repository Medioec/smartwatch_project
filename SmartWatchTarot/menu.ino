...
void startScreen(int* currX, int* currY){
  ...
  display.setCursor(*currX, *currY);
  char* HeaderText = "Tarot Reading";
  display.setFont(liberationSans_10ptFontInfo); 
  display.fontColor(WHITE,BLACK);
  int fontHeight = display.getFontHeight();
  int fontWidth = display.getPrintWidth(HeaderText);
  display.setBrightness(10);
  display.setFlip(true);
...
//------------------------------------------------------
/*
void printDisplay(int* currX, int* currY) {
...
static const char PROGMEM mainMenuStrings3[] = "Tarot Reading";
